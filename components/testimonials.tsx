...
export default function Testimonials() {
  ...
  const navigate = useCallback((to: number, direction: 1 | -1) => {
    setTx(prev => prev !== null ? prev : { from: currentRef.current, to, direction, phase: 'start' })
  }, [])
  ...
  useEffect(() => {
    ...
    if (tx.phase === 'end') {
      const timer = setTimeout(() => { setCurrent(tx.to); setTx(null) }, 500)
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      setTx(prev => prev !== null ? prev : { from: c, to: next, direction: 1, phase: 'start' })
    }, 4000)
  ...
  const prev = useCallback(() => {
    navigate((currentRef.current - 1 + testimonials.length) % testimonials.length, -1)
  }, [navigate])
  ...
  const slideStyle = (role: 'outgoing' | 'incoming'): CSSProperties => {
    ...
    if (role === 'outgoing') {
      return {
        ...base,
        transform: tx.phase === 'start' ? 'translateX(0)' : `translateX(${-d * 80}px)`,
        opacity: tx.phase === 'start' ? 1 : 0,
        transition: tx.phase === 'end' ? 'transform 0.5s ease-in-out, opacity 0.5s ease-in-out' : 'none',
      }
    }
    return {
      ...base,
      transform: tx.phase === 'start' ? `translateX(${d * 80}px)` : 'translateX(0)',
              ...
              {testimonials.map((_, i) => (
                <button
                  ...
                  className={`w-2 h-2 transition-colors ${
                    i === activeIndex ? 'bg-[#3DDC97]' : 'bg-[#2A2A2A] hover:bg-[#888888]'
                  }`}
